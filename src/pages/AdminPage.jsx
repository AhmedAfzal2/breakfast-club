...
function AdminPage() {
  ...
  const fetchData = async (isPolling = false) => {
    ...
    try {
      // Fetch Reservations
      const resData = await reservationsApi.getAllReservations();
      setReservations(resData);

      // Fetch Orders
      const ordData = await ordersApi.getAllOrders();
      ...
      if (!isPolling) {
        const menuData = await menuApi.getAllMenuItems();
              ...
              <div className="orders-list">
                {orders.map((order, index) => (
                  <div
                    ...
                    } ${exitingOrders.includes(order._id) ? "exiting" : ""}`}
                      ...
                      <Items
                        items={selectedOrder.items.map((item) => ({
                          ...
                          src:
                            item.menuItemId?.src ||
                            item.src ||
                            menuItems.find((m) => m.name === item.name)?.src,
                        }))}
                        readOnly={true}
                      />
                    </div>
